ServerEvents.recipes((event) => {
  event.replaceInput(
    //All drawers will take a bronze gear to make
    { mod: "functionalstorage" },
    "#c:chests/wooden",
    "modern_industrialization:bronze_gear"
  );
  event.replaceInput(
    //Fluid storage will take a Stainless Steel Tank
    { mod: "functionalstorage" },
    "minecraft:bucket",
    "modern_industrialization:stainless_steel_tank"
  );
  event.replaceInput(
    //All pistons are replaced with pistons ;p
    { mod: "functionalstorage" },
    "minecraft:piston",
    "modern_industrialization:piston"
  );
  event.replaceInput(
    //replace the planks for stainless steel plates in Ender Drawers
...
let tiers = ["copper", "gold", "diamond", "netherite"];
let customs = ["Steel", "Aluminum", "Stainless Steel", "Titanium"];
ItemEvents.modifyTooltips((event) => {
  for (let i = 0; i < tiers.length; i++) {
    let id = `functionalstorage:${tiers[i]}_upgrade`;

    event.modify(id, (tooltip) => {
      tooltip.removeLine(0);
      tooltip.insert(0, Text.of(`${customs[i]} Upgrade`));
    });
  }
});
